...
class RaySampler(object):
    ...
    def _select_index_train(self, plane):
        """Select plane's index to render for train mode.
        Args:
            plane (torch.Tensor): Plane to select, shape like
                ``[batch_size, H*W, 3/4]``.

        Returns:
            torch.Tensor: Selected index for training, shape like
                ``[batch_size, n_points]``.
        """
    ...
    def _select_index_eval(self, plane):
        """Select plane's index to render for evaluation mode.
        ...
        Returns:
            torch.Tensor: Selected index for evaluation.
        """
        batch_size, total_points = plane.shape[:2]
        selected_idx = torch.arange(total_points)[None,
                                                  ...].repeat(batch_size, 1)
        return selected_idx

    def _sample_points_train(self, plane, index):
        """Sample coordinates on plane with the given ``index``.

        Args:
            plane (torch.Tensor): Plane to select, shape like
                ``[batch_size, H*W, 3/4]``.
            index (torch.Tensor): Selected index of the coordinates, shape like
                ``[batch_size, n_points]``.

        Returns:
            torch.Tensor: Selected coordinates on plane, shape like
                ``[batch_size, n_points, 3/4]``.
        """
        # same usage as `torch.gather`
        if torch.__version__ >= '1.9.0':
            return torch.take_along_dim(plane, index[..., None], dim=1)
        n_coors = plane.shape[-1]
        return torch.gather(
            plane, index=index[..., None].expand(-1, -1, n_coors), dim=1)
    ...
    def _sample_pixels_train(self, image, index):
        """
        Args:
            image (torch.Tensor): The image to be samples, shape like
                ``[batch_size, 3, H, W]``.
            index (torch.Tensor): The index of the selected pixels, shape like
                ``[batch_size, n_points]``.
        Returns:
            torch.Tensor: Selected
        """
        return torch.gather(
            image, index=index[..., None].expand(-1, -1, 3), dim=1)
    ...
    def vis_pixels_on_image(self, image, points, alpha=None):
        ...
        Args:
            ...
            points (torch.Tensor): The coordinates on the image plane, range in
                [0, H] and [0, W], and shape like
                ``[batch_size, n_points, 3/4]``. We use this to generate mask.
        ...
        x_points = points[..., 0].type(torch.LongTensor)
        y_points = points[..., 1].type(torch.LongTensor)

        # TODO: may be we have better way
        for idx in range(mask.shape[0]):
            mask[idx, ..., x_points[idx], y_points[idx]] = 1

        pixels = image * mask

        vis_dict = dict(selected_mask=mask, selected_pixels=pixels)
        if alpha is not None:
            canvas = image * (1 - alpha) + (1 - mask) * alpha
            vis_dict['selected_pixels_alpha'] = canvas
...
class PrecropRaySampler(RaySampler):
    ...
    def _get_index_to_select(self, plane):
        ...
        Args:
            plane (torch.Tensor): [batch_size, H*W, 3/4]

        Returns:
            torch.Tensor: Index under the current precrop parameters, shape
                like [batch_size, H*W*precrop_frac*precrop_frac].
        """
        H = self.H_range[1] - self.H_range[0]
        W = self.W_range[1] - self.W_range[0]
    ...
    def _select_index_train(self, plane):
        """
        Args:
            plane (torch.Tensor): shape like ``[batch_size, H*W, 3/4]``.

        Returns:
            torch.Tensor: Selected index in the full image space, shape like
                ``[batch_size, n_points]``.
        """
        ...
        return torch.gather(index_to_select, index=index, dim=1)
    ...
    def _select_index_eval(self, plane):
        ...
        Returns:
            torch.Tensor: All index under current precrop parameter, shape like
                ``[batch_size, H*W*precrop_frac*precrop_frac]``.
        """
        index_to_select = self._get_index_to_select(plane)
        return index_to_select
