...
@torch.no_grad()
def inr_layer_swapping(swapped_net,
                       target_net,
                       gamma_target,
                       swapped_layers,
                       verbose=True):
...
def get_translate_rotate(r=1,
    ...
    for idx, beta in enumerate(
            np.linspace(0, 2 * math.pi * periods, num_samples)):
...
def get_yaw_camera_pos_and_lookup(
    r=1,
    num_samples=36,
):
    xyz = np.zeros((num_samples, 3), dtype=np.float32)
...
def scatter_points(idx_grad, points_grad, idx_no_grad, points_no_grad,
                   num_points):
    ...
    idx_grad_out = idx_grad.expand(points_grad.shape[0], -1,
                                   points_grad.shape[-1])
    points_all.scatter_(dim=1, index=idx_grad_out, src=points_grad)

    idx_no_grad = rearrange(idx_no_grad, 'n -> 1 n 1')
    idx_no_grad_out = idx_no_grad.expand(points_no_grad.shape[0], -1,
                                         points_no_grad.shape[-1])
...
def gather_points(points, idx_grad):
    ...
    if points.dim() == 4:
        ...
        idx_grad = idx_grad.expand(points.shape[0], -1, points.shape[-2],
                                   points.shape[-1])
...
def batch_scatter_points(idx_grad, points_grad, idx_no_grad, points_no_grad,
                         num_points):
    """
...
def batch_gather_points(points, idx_grad):
    """
    ...
    if points.dim() == 4:
        ...
        idx_grad = idx_grad.expand(points.shape[0], -1, points.shape[-2],
                                   points.shape[-1])
        sampled_points = torch.gather(
            points, dim=1, index=idx_grad, sparse_grad=False)
    elif points.dim() == 3:
        idx_grad = rearrange(idx_grad, 'b n -> b n 1')
        idx_grad = idx_grad.expand(points.shape[0], -1, points.shape[-1])
        sampled_points = torch.gather(
...
def get_initial_rays_trig(bs, num_steps, fov, resolution, ray_start, ray_end,
                          device):
    """this function build a plane shape in [-1, 1] and place at -focal on
    z-axis Returns sample points, z_vals, and ray directions in camera space.
...
def get_world_points_and_direction(batch_size,
    ...
    ret = (transformed_points, transformed_ray_directions_expanded,
           transformed_ray_origins, transformed_ray_directions, z_vals, pitch,
           yaw)
    return ret
